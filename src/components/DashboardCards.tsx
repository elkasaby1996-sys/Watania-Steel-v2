...
export function DashboardCards() {
  ...
  return <section className="metrics-strip" aria-label="Daily production totals" aria-busy={isLoadingMetrics}>
    {items.map(({ label, value, unit, note, icon: Icon }, index) => <div className={`metric-item${index === 3 ? ' metric-item-total' : ''}`} key={label}>
      <div className="metric-label"><span>{label}</span><span className="metric-icon"><Icon size={18} strokeWidth={1.5} /></span></div>
